...
!-------------------- namelist -----------------------------------------
   ...
   real :: t_zero=315., t_strat=200., delh=60.0, delv=10., eps=0., sigma_b=0.7, CHI=0.0
   real :: P00 = 1.e5
...
!modified by cw 9/16/04
   namelist /hs_forcing_nml/  t_zero, t_strat, delh, delv, eps, CHI, &
...
!modified by cw 9/16/04
! original:
!      t_star(:,:) = t_zero - delh*(sin_lat - sin(phi0))**2 + sin(phi0)**2
! modified by cw 1/29/12
      t_star(:,:) = t_zero - delh*(sin_lat - sin(phi0))**2 + delh*sin(phi0)**2 - 20.0*CHI*sin_lat
! end cw modification
...
!modified by cw 9/16/04
         the   (:,:) = t_star(:,:) - delv*(cos(lat-phi0))**2*log(p_norm(:,:))
!end cw modification
...
!  ----- compute damping -----
         sigma(:,:) = p_full(:,:,k)*rps(:,:)
! TODO pchan 20190721 Kidston 2011 N_increased run
         where (sigma(:,:) > 0.85)
           teq(:,:,k) = teq(:,:,k) - 3.0
         endwhere
! end pchan 20190721
...
!modified by cw 9/16/04
!           tdamp(:,:,k) = tka + cos(lat - phi0)**8*tfactr(:,:)
!end cw modification
